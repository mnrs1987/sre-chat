...
const TypingText: React.FC<{
  ...
  onTypingStart?: () => void;
  onTypingEnd?: () => void;
}> = ({ text = '', speed = 5, shouldAnimate = true, onTypingProgress, onTypingStart, onTypingEnd }) => {
  const [displayed, setDisplayed] = useState(shouldAnimate ? '' : text);
  const hasCompletedRef = useRef(false);
  useEffect(() => {
    if (!text) { setDisplayed(''); return; }
    // If we shouldn't animate (e.g. streaming or already done), just show text
  ...
  return <pre style={{ whiteSpace: 'pre-wrap', margin: 0, fontFamily: 'inherit', lineHeight: 1.5, fontSize: '13px' }}>{displayed}</pre>;
};
...
export const FloatingWindow: React.FC<PanelProps<Options>> = ({ options, height }) => {
  ...
  try {
    ...
    // 1. Prepare Headers - Avoid 'application/json' for a moment if fetching external
    // to see if it bypasses the CORS preflight block.
    ...
    const fetchOpts: RequestInit = {
      method,
      headers,
      mode: 'cors', // Crucial for external APIs
      credentials: 'omit',
    };

    if (method !== 'GET') {
      headers['Content-Type'] = 'text/plain'; // Using text/plain can sometimes bypass OPTIONS preflight
      fetchOpts.body = JSON.stringify({ query, model: selectedModel, stream: true });
    }

    const res = await fetch(options.apiUrl, fetchOpts);
    ...
    if (contentType.includes('text/event-stream') && res.body) {
      ...
      while (true) {
        ...
        if (done) {
          // Final prettify attempt
          let finalFormatted = accumulatedText;
          try {
            const parsed = JSON.parse(accumulatedText);
            finalFormatted = JSON.stringify(parsed, null, 2);
          } catch (e) {}

          setMessages((prev) => prev.map((m) =>
            m.id === assistantMsgId ? { ...m, text: finalFormatted, isStreaming: false } : m
          ));
          break;
        }

        const chunk = decoder.decode(value, { stream: true });
        // Clean up common streaming markers
        const cleanChunk = chunk.replace(/^data: /gm, '').replace(/data: \[DONE\]/g, '');
    ...
    // --- SINGLE RESPONSE CASE ---
      ...
      setLoading(false);
      setMessages((p) => [...p, { id: assistantMsgId, text: formatted, time: timestamp(), isStreaming: false }]);
    }
  } catch (err: any) {
    setMessages((p) => [...p, { id: makeId(), text: `❌ Connection Error: ${err.message}`, time: timestamp() }]);
    setLoading(false);
  } finally {
    setIsTyping(false);
    scroll();
