...
// --- Component: High-Performance Typing Animation Engine ---
const TypingText: React.FC<{
  ...
  // Sync callbacks to mutable refs to fully insulate the loop from parent re-renders
  ...
  useEffect(() => { onTypingEndRef.current = onTypingEnd; }, [onTypingEnd]);
  useEffect(() => { onTypingProgressRef.current = onTypingProgress; }, [onTypingProgress]);
    ...
    try {
      ...
      if (reader) {
        ...
        while (true) {
          ...
          // Tokenizer: Handle both raw string streams and formal Server-Sent Events (SSE) packets cleanly
          ...
          for (const line of lines) {
            ...
            if (trimmed.startsWith('data: ')) {
              ...
              try {
                ...
                // SSE parse mismatch fallback
              }
            } else {
              // Direct string buffer fallback
              assistantText += line + '\n';
            }
          }

          setMessages((prev) => prev.map((m) => (m.id === assistantMsgId ? { ...m, text: assistantText.trim() } : m)));
          scroll();
        }

        // --- Stream Completed Transitions ---
        let finalizedText = assistantText.trim();
        if (finalizedText.startsWith('```')) {
          finalizedText = finalizedText.replace(/^```(?:json)?\n?/, '').replace(/```$/, '').trim();
        }
        try {
          finalizedText = JSON.stringify(JSON.parse(finalizedText), null, 2);
        } catch {
          // Keep as plaintext
        }

        setMessages((prev) => prev.map((m) => (m.id === assistantMsgId ? { ...m, text: finalizedText, isStreaming: false } : m)));
        setAnimatedIds((prev) => new Set(prev).add(assistantMsgId));
        setIsTyping(false);

      } else {
        // --- Static Request Fallback ---
        const raw = await res.text();
        let finalizedText = raw.trim();
        if (finalizedText.startsWith('```')) {
          finalizedText = finalizedText.replace(/^```(?:json)?\n?/, '').replace(/```$/, '').trim();
        }
        try {
        ...
        setMessages((p) => [...p, { id: assistantMsgId, text: finalizedText, time: timestamp(), isUser: false, isStreaming: false }]);
      }
    } catch (err: any) {
